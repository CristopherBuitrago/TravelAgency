...
public class CreateTcPaymentView {
    // get the application and scanner
    private final CreateTcPaymentUseCase createTcPaymentUseCase;
    private final Scanner scanner;
    private static final int MAX_NUMBER_CARD = 10;

    // constructor
    public CreateTcPaymentView(CreateTcPaymentUseCase createTcPaymentUseCase, Scanner scanner) {
        this.createTcPaymentUseCase = createTcPaymentUseCase;
    ...
    public void start () {
        while (true) {
            try {
                // intro message
                System.out.println("CREATE CREDIT CARD PAYMENT");
                // get the data
                ...
                if (cardNumber.length() <= MAX_NUMBER_CARD) {
                    ...
                    String response = createTcPaymentUseCase.execute(payment);
